...
export class ConnectionManager {
    ...
    static async handleGoogleCalendarConnection({
        ...
        try {
            console.log('Handling Google calendar connection for user:', userId);
            ...
            if (!account.accountId) {
                console.error('No Google account ID available for calendar sync');
                return;
            }
            ...
            console.log('handleGoogleCalendarConnection completed successfully');
        } catch (error) {
            console.error('Error handling Google calendar connection:', error);
    ...
    async syncUserCalendars(): Promise<void> {
        try {
            console.log('Syncing calendars for user:', {
                userId: this.userId,
            ...
            if (!calendars) {
                console.error('Failed to fetch calendars from Google');
                return;
            }

            console.log(
                `Found ${calendars.length} calendars for user ${this.userId}:`,
            ...
            console.log(`Finished syncing ${calendars.length} calendars for user ${this.userId}`);
        } catch (error) {
    ...
    private async storeCalendars(calendars: GoogleCalendar[]): Promise<void> {
        for (const calendar of calendars) {
            console.log(`Inserting calendar: ${calendar.summary} (${calendar.id})`);
            ...
            console.log(`Successfully stored calendar: ${calendar.summary}`);
        }
    }
